...
{
public : 
	typedef vector<POINT> _vFrameList;	//프레임 접근은 임의 접근이 가능해야 함.
	typedef vector<int> _vPlayList;		//프레임플레이 리스트
...
public:
	...
	//이미지 총 가로세로랑 프레임당 가로세로
	HRESULT init( int totalW, int totalH, int frameW, int frameH );

	void setDefPlayFrame( BOOL reverse = FALSE, BOOL loop = FALSE ); //기본셋팅

	void setPlayFrame( int* playArr, int arrLen, BOOL loop = FALSE );	//플레임 배열로 셋팅
	void setPlayFrame( int start, int end, BOOL reverse = FALSE, BOOL loop = FALSE );//프레임 시작과 끝으로 셋팅
